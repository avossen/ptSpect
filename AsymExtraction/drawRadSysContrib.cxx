...
int main(int argc, char** argv)
{
      ...
      for(int j=0;j<numFiles;j++)
	{
	  for(int zbin=0;zbin<maxFirstBin;zbin++)
	      ...
	      for(int k=0;k<numKtBins;k++)
		{
		  ...
		  double kTMean=plotters[i+1][j]->plotResults[resIdx].kTMeans[k];
		  double kTVal=plotters[i+1][j]->plotResults[resIdx].kTValues[k];
		  double kTValWRad=plotters[i][j]->plotResults[resIdx].kTValues[k];
  ...
  for(int uc=0;uc<2;uc++)
      ...
      for(int i=0;i<minCCount;i++)
	{
      ...
      //      p->SetLogy();
	  graphs[uc][0][i]->GetYaxis()->SetRangeUser(0,maxVals[i]*1.5);
	  graphs[uc][0][i]->GetXaxis()->SetRangeUser(0,maxValsX[i]*1.2);
	  graphs[uc][0][i]->Draw("AP");      
	  for(int j=1;j<numFiles;j++)
	    {
	      graphs[0][j][i]->Draw("SAME P");      
	    }
	}
      auto legend=new TLegend(0,0,1.0,1,0);
      for(int i=0;i<numFiles;i++)
	{
	  legend->AddEntry(graphs[0][i][0]->GetName(),names[i].c_str(),"lep");
	}
      c.cd(12);
      legend->Draw();
      if(uc==1)
