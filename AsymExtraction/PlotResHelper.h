...
PlotResults&  add(PlotResults& lhs, const PlotResults& rhs, float prefactor=1.0)
{
  cout <<"prefactor: "<< prefactor <<endl;
      ...
      for(int i=0;i<maxKtBins;i++)
	{
	  ...
	  intRhsCount+=(prefactor*rhs.kTValues[i]);
	  ...
	  if(lhs.kTValues[i]!=0 && rhs.kTValues[i]!=0)
	    {
	      lhs.kTMeans[i]=1.0/(lhs.kTValues[i]+(prefactor*rhs.kTValues[i]))*(lhs.kTMeans[i]*lhs.kTValues[i] + (prefactor)*rhs.kTMeans[i]*rhs.kTValues[i]);
	    }
	  //just assign the rhs side. Assume that we are not really adding files 
	  //	  if(weakDecayFraction[i]!=0 && rhs.weakDecayFraction[i]!=0)
	      if((lhs.kTValues[i]+(prefactor*rhs.kTValues[i]))>0)
	    {
	      ...
	      if(::isnan(rhsWeakDecays))
		{
	      ...
	      //	      total weak decays
	      lhs.weakDecayFraction[i]=origDecayFraction*lhs.kTValues[i]+(prefactor*rhsWeakDecays*rhs.kTValues[i]);
	      lhs.weakDecayFraction[i]/=(lhs.kTValues[i]+(prefactor*rhs.kTValues[i]));
	  ...
	  lhs.kTValues[i]+=(prefactor*rhs.kTValues[i]);
	  lhs.kTValues1[i]+=(prefactor*rhs.kTValues1[i]);
	  lhs.kTValues2[i]+=(prefactor*rhs.kTValues2[i]);

	  lhs.kTSysUncertainties[i]=sqrt(lhs.kTSysUncertainties[i]*lhs.kTSysUncertainties[i]+(prefactor*prefactor*rhs.kTSysUncertainties[i]*rhs.kTSysUncertainties[i]));
	  lhs.kTUncertainties[i]=sqrt(lhs.kTUncertainties[i]*lhs.kTUncertainties[i]+(prefactor*prefactor*rhs.kTUncertainties[i]*rhs.kTUncertainties[i]));
	  lhs.kTUncertainties1[i]=sqrt(lhs.kTUncertainties1[i]*lhs.kTUncertainties1[i]+(prefactor*prefactor*rhs.kTUncertainties1[i]*rhs.kTUncertainties1[i]));
	  lhs.kTUncertainties2[i]=sqrt(lhs.kTUncertainties2[i]*lhs.kTUncertainties2[i]+(prefactor*prefactor*rhs.kTUncertainties2[i]*rhs.kTUncertainties2[i]));
...
//rhscount already has prefactor
