...
do
for spec in tautau eemm eeuu eess eecc
do
echo looking at nonQQ_$spec\_$res\_ex$ex
for d in `ls nonQQ_$spec\_$res\_ex$ex/*.sh`
do
 bsub -q s $PWD/$d
done
    num_jobs=`bjobs | wc -l`
...
for pt in kT qT 
do
    mkdir mDstOut/nonQQStudies/nonQQ_$spec\_$res\_ex$ex\_$pt/
    ./TwoHadAsymsCMod  ../groupData/ptSpect/nonQQStudies/nonQQ_$spec\_$res\_ex$ex/ mDstOut/nonQQStudies/nonQQ_$spec\_$res\_ex$ex\_$pt/ $pt mc
done
#delete large output
#rm ../groupData/ptSpect/nonQQStudies/nonQQ_$spec\_$res\_ex$ex/*.root
rm ~/.lsf/*.out
cd /home/belle/vossen/myProjects/ptSpect/batchStuff/nonQQStudyScripts
done
done
done
